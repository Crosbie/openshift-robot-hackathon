...
* Edit the link and add your username and passwort to it for push authentication. It should now look like this: `*http://<GUID>:<PASSWORD>@gogs-lab-infra.apps.berlin-618f.openshiftworkshop.com/<GUID>/<GUID>-python*`
* Click on `*Import*`
...
. Click on `*Next*`
. As `Application Name` use `*<GUID>-phython*` and as `GIT Repository` use `*http://gogs-lab-infra.apps.43d7.events.opentlc.com/<GUID>/<GUID>-php*`
